...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-vkvm-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Virtual KVM Policy.",

	"EnableLocalServerVideo": true,
	"EnableVideoEncryption": true,
	"Enabled": true,
	"RemotePort": 2068,
	"TunneledKvmEnabled": false
}
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/kvm/Policies/%s", moid), expectedJSONTemplate, vars)
}
